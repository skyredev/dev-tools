...
class ModifyEntity(BaseCommand):
    ...
    def get_value_instance(value_type, value_name, section_path):
        try:
            if section_path[0] == 'fields':
                value_module = importlib.import_module(f"DevTools.EntityMetadata.Fields.{value_type}")
            else:
                value_module = importlib.import_module(f"DevTools.EntityMetadata.Links.{value_type}")
            ...
            if confirm == "Yes":
                ...
                for language in language_files:
                    translation_filepath = self.FileManager.ensure_file_exists(
                        self.PathManager.get_i18n_path(entity_name, language))
    ...
    def translate_values(self, entity_name):
        ...
        filepath = self.FileManager.ensure_file_exists(self.PathManager.get_i18n_path(entity_name, opposite_language))
